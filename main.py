...
from utils import (detect_anomaly, get_cities, get_colors_from_anomalies,
                   get_current_season, get_current_temperature,
                   get_seasonal_stats, merge_seasonal_stats)
...
if cities:
    ...
    if city and api_key:
        ...
        temperature = asyncio.run(get_current_temperature(city, api_key))
        anomaly = detect_anomaly(
            seasonal_stats=city_info.seasonal_stats, city=city, temperature=temperature
        )
        ...
        st.write(f"Температурный тренд {temp_trend}")

        st.text("График аномальных температур в исторических данных")
        st.scatter_chart(
            city_info.total_info,
            x="timestamp",
            y="temperature",
            color="color",
        )
